...
static int BinarySearchTreeElementComparator(void *element1, void *element2) {
    log_trace("BinarySearchTreeElementComparator <-");
    BinarySearchTreeElement *e1 = (BinarySearchTreeElement *) element1;
    BinarySearchTreeElement *e2 = (BinarySearchTreeElement *) element2;

    if(e1->key > e2->key){
        log_info("elemento1 É MAIOR que elemento2!");
        log_debug("elemento1: %d, emelemnto2: %d",e1->key, e2->key);
        return 1;
    }

    if(e1->key < e2->key){
        log_info("elemento1 É MENOR que elemento2!");
        log_debug("elemento1: %d, emelemnto2: %d",e1->key, e2->key);
        return -1;
    }

    if(e1->key == e2->key){
        log_info("os elementos SÃO IGUAIS!");
        log_debug("elemento1: %d, emelemnto2: %d",e1->key, e2->key);
        log_trace("BinarySearchTreeElementComparator ->");
        return 0;
    }
...
int main(int argc, char **argv) {
    ...
    assert(el != NULL);//assert é uma função que testa se uma expressão é valida
    el->key = 45;
